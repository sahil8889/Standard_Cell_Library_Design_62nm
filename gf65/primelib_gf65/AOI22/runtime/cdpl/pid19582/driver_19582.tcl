if { [catch {
set_location /home/eng/s/sxv240035/cad/gf65/primelib_gf65/AOI22
log_info "[clock format [clock seconds]]: Sourcing environment settings"
set t0 [clock seconds]
set sis_log_file "/var/tmp/pl.860278.W$::env(CDPL_WORKERID).[lindex [split [info hostname] '.'] 0].[pid].log"
if { [info exists ::env(SGE_TASK_ID)] } {
  if { $::env(SGE_TASK_ID) != "undefined" } {
    set sis_log_file "/var/tmp/pl.860278.W$::env(SGE_TASK_ID).[lindex [split [info hostname] '.'] 0].[pid].log"
  }
}
...
set_config_opt -- liberty_multi_rail_format not_specified
...
if {[catch {
  ...
  reset_log_file
  if { [log_num_errors] > 0 } {
    if { [catch {exec mv -f $sis_log_file /home/eng/s/sxv240035/cad/gf65/primelib_gf65/AOI22/runtime/cdpl/pid19582} mv_err] } {
      log_warning $mv_err
    }
  } else {
